request = require 'request'
cheerio = require 'cheerio'

module.exports = (name, number, callback) ->
  form =
    '__VIEWSTATE': ''
    '__EVENTVALIDATION': ''

    'ctl00$ContentPlaceHolder1$ddlType': '2'
    'ctl00$ContentPlaceHolder1$textfieldnumber': number
    'ctl00$ContentPlaceHolder1$textfieldname': name
    'ctl00$ContentPlaceHolder1$BtnQueren.x': '22'
    'ctl00$ContentPlaceHolder1$BtnQueren.y': '13'

  opts =
    url: 'http://zsb.neau.edu.cn/Query.aspx'
    method: 'POST'
    form: form

  request opts, (err ,res) ->
    return callback err if err

    if res?.statusCode isnt 302
      return callback new Error 'statusCode error'

    resultUrl = res.headers.location

    request resultUrl, (err, res, data) ->
      return callback err if err

      if res?.statusCode isnt 200
        return callback new Error 'statusCode error'

      $ = cheerio.load data

      result =
        name: $('#ctl00_ContentPlaceHolder1_lblxm').text()
        sex: $('#ctl00_ContentPlaceHolder1_lblxb').text()
        ksh: $('#ctl00_ContentPlaceHolder1_lblzkzh').text()
        province: $('#ctl00_ContentPlaceHolder1_lblsf').text()
        grade: $('#ctl00_ContentPlaceHolder1_lblzf').text()
        rank: $('#ctl00_ContentPlaceHolder1_lblmc').text()
        bc: $('#ctl00_ContentPlaceHolder1_lblbc').text()
        info: {}

      for i in [1..10]
        data = $ "#ctl00_ContentPlaceHolder1_lblData#{i}"
        unless data
          break

        dataName = data.parent().prev().text().trim()
        result.info[dataName] = data.text()

      callback null, result

module.exports.getTip = (callback) ->
  request 'http://zsb.neau.edu.cn/Query.aspx', (err, res, data) ->
    return callback err if err

    if res?.statusCode isnt 200
      return callback new Error 'statusCode error'

    $ = cheerio.load data
    tip = $('.zhujie').text()

    callback null, tip
